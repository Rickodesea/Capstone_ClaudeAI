...
\[
  \underbrace{\text{Prediction}}_{\mu_{i,j,r},\,\Sigma_r,\,d_{i,j,r},\,N_{i,t}}
  \;\longrightarrow\;
...
\begin{longtable}{@{}p{2.2cm}p{11.5cm}@{}}
  ...
  $\mu_{i,j,r}$ & Mean predicted resource \emph{usage} of workload $(i,j)$ for
                   resource $r$ --- expected actual usage per job, averaged over
                   the planning slot window (e.g.\ a 4-hour block).
                   \textit{Source: prediction model output --- Workload Profiler}
                   (from \texttt{InstanceUsage.average\_usage} aggregated to
                   4-hour windows per workload class per tenant).
  ...
  $\Sigma_r$    & Empirical covariance matrix of resource $r$ across all workloads,
                   ...
                   semi-definite.
                   \textit{Source: prediction model output} (estimated from
                   \texttt{InstanceUsage.random\_sampled\_usage}).
  ...
  $N_{i,t}$      & Expected number of active jobs (workloads) for tenant $i$
                   ...
                   \textit{Source: prediction layer --- Job Count Predictor.}
                   Model type: descriptive statistics or simple regression on
                   historical \texttt{InstanceEvents} \texttt{SUBMIT} records
                   per tenant per slot (day-of-week $\times$ time-of-day bucket).
                   For Autopilot/HPA-managed workloads, estimated from
                   auto-scaled task counts in the historical trace. \\
  \bottomrule
\end{longtable}
...
\begin{tabular}{@{}lll@{}}
  ...
  $N_{i,t}$     & \texttt{InstanceEvents}  & \texttt{SUBMIT} event count per tenant per 4-hour time-of-day bucket (prediction layer input) \\
  \bottomrule
\end{tabular}
\end{center}
