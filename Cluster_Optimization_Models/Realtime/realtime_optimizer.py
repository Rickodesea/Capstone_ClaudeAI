...
Per-call solver override
────────────────────────
    Pass solver_id="SCIP" to solve() to override the module-level SOLVER_ID
    for one call without mutating global state. Used by optimizer_iterative.py.
...
def solve(
    ...
    solver_id     : override module-level SOLVER_ID for this call only (None = use SOLVER_ID)
    ...
    sid = (solver_id if solver_id else SOLVER_ID).upper()
