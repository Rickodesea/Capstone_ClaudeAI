...
export function HUD({ hud, interval, planAheadInterval, useEffective }: Props) {
  const displayPct = useEffective ? hud.eff_utilization_pct : hud.mem_utilization_pct
  const label      = useEffective ? 'EFF' : 'CAP'
  const memColor   =
  ...
  return (
    <div className="h-full flex flex-col px-2 py-2">
      <div className="flex-1 bg-slate-900 rounded-xl border border-slate-700/40 flex flex-col gap-3 px-4 py-4">
        ...
        {/* Memory % — large display, reflects current Eff/Cap toggle */}
